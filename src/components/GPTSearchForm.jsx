...
const GPTSearchForm = () => {
  ...
  const handleClick = async()=> {
    ...
    const suggestedMovies = ["Andaz Apna Apna","Hera Pheri", "Chupke Chupke", "Jaane Bhi Do Yaaro", "Padosan"];

    const promiseArray = suggestedMovies.map(movie => searchMovieTMDB(movie));

    const tmdbResults = await Promise.all(promiseArray);

    dispatch(addGptMovieResults({movieNames: suggestedMovies, movieResults: tmdbResults}));
  }
  return (
    <div className='pt-[10%] flex justify-center '>
        <form className='w-1/2 bg-black grid grid-cols-12 ' onSubmit={(e) => e.preventDefault()}>
            <input ref={inputText} placeholder={lang[langSelected].gptSearchPlaceholder} type='text'
...
// https://api.themoviedb.org/3/search/Andaz%20Apna%20Apna?include_adult=false&language=en-US&page=1